...
/// Artifact visibility policy shared by HTTP serving and standalone-file search.
/// Author-document scanning deliberately uses `ScanIgnore` instead.
...
impl VaultIgnore {
    ...
    pub fn load(root: impl AsRef<Path>) -> Result<Self, ::ignore::Error> {
...
fn is_builtin_ignore_path(root: &Path, path: &Path) -> bool {
    use crate::scan::DEFAULT_IGNORED_DIRS;
